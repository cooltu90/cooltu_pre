...
public interface BindHandlerModelInterface {
    ...
    default List<String> getTempLinesArray() {
        ...
        lines.add("");
        lines.add("");
        lines.add("        private cooltu.lib4j.data.map.ListValueMap<Integer, com.codingtu.cooltu.processor.lib.form.FormLink> links;");
        lines.add("");
        lines.add("        private cooltu.lib4j.data.map.ListValueMap<Integer, com.codingtu.cooltu.processor.lib.form.FormLink> getLinks() {");
        ...
        lines.add("        }");
        lines.add("");
        lines.add("        public void addLink(int index, com.codingtu.cooltu.processor.lib.form.FormLink link) {");
        lines.add("            getLinks().get(index).add(link);");
        lines.add("        }");
        lines.add("");
        lines.add("        private void link(int id) {");
        lines.add("            cooltu.lib4j.ts.Ts.ls(getLinks().get(id), new cooltu.lib4j.ts.each.Each<com.codingtu.cooltu.processor.lib.form.FormLink>() {");
        lines.add("                @Override");
        lines.add("                public boolean each(int position, com.codingtu.cooltu.processor.lib.form.FormLink formLink) {");
